...
public class King extends Piece {
	...
	public List<Cell> move(Cell[][] state, int x, int y) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (horizontalMoveAvailable && verticalMoveAvailable
					&& chechPossibleMoves(i, state, posx, posy)) {
	...
	public boolean chechPossibleMoves(int i, Cell[][] state, int[] posx,
			int[] posy) {
		return (state[posx[i]][posy[i]].getpiece() == null || state[posx[i]][posy[i]]
	...
	public int isIndangerXandY(int x, int y, Cell[][] state) {
		if (state[x][y].getpiece() == null)
			...
			if ((state[x][y].getpiece() instanceof Rook)
					|| (state[x][y].getpiece() instanceof Queen))
				return 1;
			else
				return 0;
		}
	...
	public int isIndangerDiagonal(Cell[][] state) {
		...
		if (state[tempHorizontalPos][tempVerticalPos].getpiece().getcolor() == this
				.getcolor())
			return 0;
		else {
	...
	public boolean isIndangerFromOpposition(int[] posx, int[] posy,
			Cell[][] state, boolean king) {
		...
		for (int i = 0; i < 8; i++) {
			if ((posx[i] >= 0 && posx[i] < 8 && posy[i] >= 0 && posy[i] < 8)
				...
				if (!king
						&& (state[posx[i]][posy[i]].getpiece() instanceof King)) {
					danger = true;
				}
	...
	public boolean checkDangerFromOpposition(int[] posx, int[] posy,
			Cell[][] state, int i) {
		return (state[posx[i]][posy[i]].getpiece() != null && state[posx[i]][posy[i]]
				.getpiece().getcolor() != this.getcolor());
	}
	...
	public boolean checkColor(Cell[][] state, int xx, int yy, int color) {
		...
		return state[xx][yy].getpiece() != null
				&& state[xx][yy].getpiece().getcolor() == color
	...
	public boolean isInDangerFromEnemyKnight(Cell[][] state) {
		int[] posx = { horizontalPos + 1, horizontalPos + 1, horizontalPos + 2,
				horizontalPos + 2, horizontalPos - 1, horizontalPos - 1,
				horizontalPos - 2, horizontalPos - 2 };
		int[] posy = { verticalPos - 2, verticalPos + 2, verticalPos - 1,
				verticalPos + 1, verticalPos - 2, verticalPos + 2,
				verticalPos - 1, verticalPos + 1 };

		return isIndangerFromOpposition(posx, posy, state, true);
	}

	public boolean isInDangerFromEnemyPawn(Cell[][] state) {
		int[] pox = { horizontalPos + 1, horizontalPos + 1, horizontalPos + 1,
				horizontalPos, horizontalPos, horizontalPos - 1,
				horizontalPos - 1, horizontalPos - 1 };
		int[] poy = { verticalPos - 1, verticalPos + 1, verticalPos,
				verticalPos + 1, verticalPos - 1, verticalPos + 1,
	...
	public boolean getColorFromHorizontalLeft(Cell[][] state) {
		if (horizontalPos > 0 && verticalPos > 0
				&& checkColor(state, horizontalPos - 1, verticalPos - 1, 1))
			return true;

		return (horizontalPos > 0 && verticalPos < 7 && checkColor(state,
				horizontalPos - 1, verticalPos + 1, 1));

	}

	public boolean getColorFromHorizontalRight(Cell[][] state) {
		if (horizontalPos < 7 && verticalPos > 0
				&& checkColor(state, horizontalPos + 1, verticalPos - 1, 0))
			return true;

		return (horizontalPos < 7 && verticalPos < 7 && checkColor(state,
				horizontalPos + 1, verticalPos + 1, 0));
	}
